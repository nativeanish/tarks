...
function ClassicDark(props: Props) {
  return (
    <div className="min-h-screen bg-[#212121] text-white py-12 px-4 sm:px-6 lg:px-8">
      <button
        id="share"
        className="fixed border-2 border-white top-4 right-4 md:top-8 md:right-8 z-10 flex items-center justify-center px-4 py-2 glass-effect text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
      >
        <svg
      ...
      <div className="max-w-md mx-auto pt-3">
        <div className="text-center space-y-8">
          {props.image && props.image.length > 0 && (
            <>
              <div className="w-32 h-32 rounded-2xl mx-auto flex items-center justify-center transform hover:rotate-3 transition-transform duration-300 shadow-lg">
                <img
                  className="w-full h-full object-cover rounded-2xl bg-white border-white border-2"
                  src={props.image}
                  alt={
          ...
          <div className="space-y-4">
            {props.name && props.name.length > 0 && (
              <>
                <h1 className="text-5xl font-semibold text-white tracking-tight hover:tracking-wide transition-all duration-300">
                  {props.name}
                </h1>
              </>
            )}
              ...
              <div className="space-y-3 flex flex-col justify-center w-full max-w-sm">
                {props.links.map((item, index) => {
                  return (
                    <div key={index}>
                      <a
                        className={`${item.className} transform hover:scale-105 transition-all duration-300 rounded-lg flex flex-row items-center justify-center space-x-4 p-3 font-medium shadow-sm hover:shadow-md mb-3`}
                        target="_blank"
