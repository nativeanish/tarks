...
var R = (r, e, t) =>
  e in r
    ...
    : (r[e] = t);
var b = (r, e, t) => R(r, typeof e != "symbol" ? e + "" : e, t);
(function () {
...
class s {
  ...
  static compareVersions(e, t, i = !1) {
    ...
    let a = Math.max(n, o),
      u = 0;
    const c = s.map([e, t], (f) => {
      const m = a - s.getVersionPrecision(f),
        O = f + new Array(m + 1).join(".0");
      return s
        .map(O.split("."), (y) => new Array(20 - y.length).join("0") + y)
        .reverse();
    });
    for (i && (u = a - Math.min(n, o)), a -= 1; a >= u; ) {
  ...
  satisfies(e) {
    ...
    if (
      (Object.keys(e).forEach((u) => {
        ...
        typeof c == "string"
          ? ((n[u] = c), (o += 1))
          : typeof c == "object" && ((t[u] = c), (i += 1));
      }),
      i > 0)
    ) {
      const u = Object.keys(t),
        c = s.find(u, (m) => this.isOS(m));
      if (c) {
        const m = this.satisfies(t[c]);
    ...
    if (o > 0) {
      const u = Object.keys(n),
        c = s.find(u, (f) => this.isBrowser(f, !0));
...
class p {
  ...
  static parse(e) {
    ...
    if (a) {
      ...
      for (let c = 0; c < 16; c += 4) {
        const f = parseInt(a.substring(2 * c, 2 * c + 8), 16);
        (u[c + 0] = f >>> 24),
          (u[c + 1] = f >>> 16),
          (u[c + 2] = f >>> 8),
          (u[c + 3] = f);
  ...
  toString() {
    ...
    for (let t = 0; t < this.bytes.length; t++)
      (e += w.charAt(this.bytes[t] >>> 4)),
        (e += w.charAt(this.bytes[t] & 15)),
        (t === 3 || t === 5 || t === 7 || t === 9) && (e += "-");
    return e;
  }
  toHex() {
    let e = "";
    for (let t = 0; t < this.bytes.length; t++)
      (e += w.charAt(this.bytes[t] >>> 4)), (e += w.charAt(this.bytes[t] & 15));
    return e;
  }
  toJSON() {
...
let S;
const U = () => T().toString(),
  T = () => (S || (S = new P())).generate(),
  F = "https://metapaths-server.onrender.com/",
  W = "metapaths.ar.io";
var v;
window.onload = async () => {
  ...
  const t = Date.now(),
    i = C.getParser(window.navigator.userAgent),
  ...
  const a = document.querySelector('meta[name="uuid"]') || "";
  v = U();
  const u = document.querySelector('meta[name="arns_name"]') || "",
    c = await fetch(`${F}/register_view`, {
      method: "POST",
    ...
    document.addEventListener("click", async (r) => {
      if (
        ...
        const e = r.target,
          ...
          n = await fetch(`${F}/register_click`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              viewId: v,
